...
# 1. 加载数据
traj_data = pd.read_csv('test_traj.csv')
jump_task_data = pd.read_csv('test_jump.csv')
    ...
    for trajectory_id, group in grouped_test:
        if len(group) > window_size:
            ...
            for i in range(len(group) - window_size - 1, len(group) - 1):
                location = group.iloc[i].coordinates.strip("[]").split(",")
                feature.append(float(location[0]))
                feature.append(float(location[1]))
            ...
            for i in range(window_size - len(group)):
                location = group.iloc[0].coordinates.strip("[]").split(",")
                feature.append(float(location[0]))
                feature.append(float(location[1]))
...
combined_df.to_csv('predicted_jump_test.csv', index=False)

print("Jump预测已完成，结果已保存到 'predicted_jump_test.csv'")
