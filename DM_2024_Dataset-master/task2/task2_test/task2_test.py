...
sim_task_data = pd.read_csv('test_sim.csv')
...
similarity_results.to_csv('similarity_results_test.csv', index=False)
...
for index, row in similarity_results.iterrows():
    ...
    plt.legend()
    plt.savefig(f'trajectory_{sim_trajectory_id}_test.png')
    plt.close()
